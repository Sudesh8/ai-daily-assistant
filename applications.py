...
def get_weather(city: str):
    """
    featches current weather for given city using api

    Args:
        city (str): city nmae(eg : Mumbai)

    returns:
    dict: weather details in jason format
    ...
    try:
        ...
        if not api_key:
            raise ValueError("OpenWeather API key not found in my.env")
...
def temperature_of_city(city):
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=system_instructions, tools=[get_weather]
        ),
    )
...
def get_news(topic: str):
    ...
    try:
        ...
        if not api_key:
            raise ValueError("NewsAPI key not found in my.env")
...
def find_local_events(city: str):
    ...
    try:
        ...
        if not api_key:
            raise ValueError("SerpAPI key not found in my.env")
